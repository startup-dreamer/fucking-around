...
abstract contract Vault is IVault, ERC4626 {
    ...
    function totalAssets() public view override returns (uint256) {
        ...
        for(uint256 i = 0; i < marketMapLength;) {
            ...
            uint256 marketTokenValue = _stakedMarketTokenValue(
                marketAddress, 
                maximize
                );
        ...
        return _asset.balanceOf(address(this)) + _getAssetTokenValueDenominatedAssetPrice(address(_asset), totalTokenValue);
    }

    /**
     * @dev returns asset token value denominated in
     */
    function _getAssetTokenValueDenominatedAssetPrice(address asset_, uint256 tokenValue_) internal view returns (uint256) {
    ...
    function _convertToShares(uint256 assets, Math.Rounding rounding) internal view override returns (uint256) {
        ...
        return supply == 0 ? assets : assets.mulDiv(totalSupply() + 10 ** _decimalsOffset(), totalAssets() + 1, rounding);
    }
     ...
     */
    function _marketTokenPrice(address market_) internal view returns(
    ...
    function _stakedMarketTokenValue(address market_, bool maximize_) internal view returns (uint256) {
        ...
        return positiveMarketTokenPrice * IERC20(market_).balanceOf(address(this));
    }

    function _toFloorUint(int256 value_) pure internal returns(uint256) {
